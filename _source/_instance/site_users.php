...
if(isset($_POST["exec_edit"]) AND trim(@$_POST["username"]) != "") {
	...
	if($user->exists(@$_POST["exec_ref"])) {
		...
		if(isset($_POST["relay"])) { $permsobj->addPerm($userid, "relay"); } else { $permsobj->removePerm($userid, "relay"); }
		if(isset($_POST["relay_own"])) { $permsobj->addPerm($userid, "relay_own"); } else { $permsobj->removePerm($userid, "relay_own"); }
		...
		if(isset($_POST["domains"])) { $permsobj->addPerm($userid, "domains"); } else { $permsobj->removePerm($userid, "domains"); }
		x_eventBoxPrep("User has been updated!", "ok", _COOKIES_);	
	} else {			
		if( strlen(trim(@$_POST["password"]))  > 8) { x_eventBoxPrep("Password needs at least 8 signs!!", "error", _COOKIES_); goto endofex; }
	...
	<div class="internal_popup">
		...
		<?php if(@$_GET["edit"] == "add") { ?> <input type="password" placeholder="Password" name="password" > <?php } ?>
				...
				<?php if($user->get($_GET["edit"])["user_rank"] != 0 OR @$_GET["edit"] == "add") { $curid = $user->get($_GET["edit"])["id"]; ?>		
					...
					<div style="float:left"><input type="checkbox" name="domains" <?php if($permsobj->hasPerm($curid, "domains")) { echo "checked"; } ?>> Manage All Domains</div><br clear="left"/>
					<div style="float:left"><input type="checkbox" name="relay" <?php if($permsobj->hasPerm($curid, "relay")) { echo "checked"; } ?>> Manage All Relays</div><br clear="left"/>
					...
					<div style="float:left"><input type="checkbox" name="logs" <?php if($permsobj->hasPerm($curid, "logs")) { echo "checked"; } ?>> View Cronlog</div><br clear="left"/>
...
<?php if($user->exists(@$_GET["delete"])) { ?>	
	<div class="internal_popup">
		<div class="internal_popup_inner">
			...
			<div class="internal_popup_content">If you delete the user, user related domains and relays will not be deleted!</div>
